...
let ctxRef = null;
let canvasRef = null;
let paletteRef = { stroke: '#404040', segment: '#f4f4f4', first: '#f4f4f4' };

function setRenderContext({ ctx, canvas, palette }) {
  ctxRef = ctx;
  canvasRef = canvas;
  paletteRef = {
    stroke: palette?.stroke ?? '#404040',
    segment: palette?.segment ?? '#f4f4f4',
    first: palette?.first ?? palette?.segment ?? '#f4f4f4',
  };
}

function getStrokeColor() {
  return paletteRef.stroke;
}

function getSegmentColor() {
  return paletteRef.segment;
}

function getFirstSoundMarkerColor() {
  return paletteRef.first;
}

function getQuadrantMetricsLocal(quadrant) {
  return getQuadrantMetrics(canvasRef, quadrant);
}
...
function drawGatiQuadrant3dInternal(config, elapsed) {
  const ctx = ctxRef;
  const canvas = canvasRef;
  const { orientation, view2d, cycleDuration } = config;
  if (!ctx || !canvas || !view2d) {
    return;
  }

  const { offsetX, offsetY, width, height, center } = getQuadrantMetricsLocal(orientation);
  ...
  const drawLineShape = () => {
    ...
    if (view2d.bounce) {
      ...
      if (cycle > 0) {
        ...
        const index = Math.floor(local / segmentDuration);
        const t = (local - index * segmentDuration) / segmentDuration;
        const current = index % 2 === 0 ? start : end;
        const next = index % 2 === 0 ? end : start;
  ...
  const drawPolygonShape = () => {
    ...
    const markerPoints = getPolygonSoundPoints(points, view2d.soundMarkers);
    markerPoints.forEach((point) => drawMarker(point, { height: eventHeight }));
    ...
    const cycle = segmentDuration * sides;
    const local = cycle > 0 ? elapsed % cycle : 0;
    const index = segmentDuration > 0 ? Math.floor(local / segmentDuration) : 0;
    const t = segmentDuration > 0 ? (local - index * segmentDuration) / segmentDuration : 0;
  ...
  const drawCircleShape = () => {
    const markers = (() => {
      if (view2d?.soundMarkers?.mode === 'count') {
        const count = Math.max(1, Math.floor(view2d.soundMarkers.count));
        return Array.from({ length: count }, (_, index) => {
          const angle = orientationAngle + (index / count) * Math.PI * 2;
          return {
            x: baseCenter.x + shapeRadius * Math.cos(angle),
            y: baseCenter.y + shapeRadius * Math.sin(angle),
          };
      ...
      return [firstMarkerBasePoint];
    })();

    markers.forEach((point) => drawMarker(point, { height: eventHeight }));

    const local = shapeCycle > 0 ? elapsed % shapeCycle : 0;
    const progress = shapeCycle > 0 ? local / shapeCycle : 0;
...
function drawJatiQuadrant3dInternal(config, elapsed) {
  ...
  const { offsetX, offsetY, width, height, center, verticalShift } =
    getQuadrantMetricsLocal(orientation);
  const origin = {
    x: offsetX + width * 0.54,
...
function drawNadaiQuadrant3dInternal(config, elapsed) {
  const ctx = ctxRef;
  const canvas = canvasRef;
...
export function drawGatiQuadrant3d({
  ctx,
  canvas,
  orientation,
  view2d,
  ...
  setRenderContext({ ctx, canvas, palette });
  drawGatiQuadrant3dInternal({ orientation, view2d, cycleDuration }, elapsed);
}

export function drawJatiQuadrant3d({
  ctx,
  canvas,
  orientation,
  view2d,
  ...
  setRenderContext({ ctx, canvas, palette });
  drawJatiQuadrant3dInternal({ orientation, view2d, cycleDuration, gatiCount }, elapsed);
}

export function drawNadaiQuadrant3d({
  ctx,
  canvas,
  orientation,
  view2d,
  ...
  setRenderContext({ ctx, canvas, palette });
  drawNadaiQuadrant3dInternal({ orientation, view2d, cycleDuration, gatiCount }, elapsed);
}
